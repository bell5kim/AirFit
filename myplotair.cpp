...
MyPlotAir::MyPlotAir(QWidget *parent)
                     :QwtPlot(parent) {

    setAutoReplot(false);

    setTitle("Comparison of AFIT against Measurement");
...
// error handler
void MyPlotAir::err_Exit(char *location, char *reason, int exit_code)
{
   cerr << endl;
...
void MyPlotAir::toggleCurve(long curveId) {
...
void MyPlotAir::usrDataFile (char *fname) {
 ...
 QTextStream(stdout) << "*** DEBUG: usrDataFile " << " p0 " << p0 << endl;
  ...
  for ( QStringList::Iterator it = cStrList.begin(); it != cStrList.end(); ++it ) {
		...
		// close file
        meas_file.close();
        QTextStream (stdout) << "Before Plotting " << endl;
		QString mLegend="";
        QString cLegend="";
        QString dLegend="";
        ...
        crvm[i] = new QwtPlotCurve("Mesured");
...
//		                     << readSettingString("Machine/Machine")
  ...
  QTextStream (stdout) << "** DEBUG: Check Point III " << xMin << " " << dmMax*1.1+(dmMax-dmMin)*0.1/8*0.8 << endl;

  // setMarkerLineStyle(textPos, QwtMarker::VLine);
  if (strncmp("x",fname,1) == 0) {
     textMarker1->setValue((double)xMin,(double)(dmMax*1.1+(dmMax-dmMin)*0.1/8*0.8);
  } 

  QTextStream (stdout) << "** DEBUG: Check Point III-1 " << endl;
  if (strncmp("y",fname,1) == 0) {
